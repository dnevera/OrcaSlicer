...
{
    switch (flavor) {
    case GCodeFlavor::gcfKlipper: return {"Default", "MZV", "ZV", "ZVD", "EI", "2HUMP_EI", "3HUMP_EI"};
    case GCodeFlavor::gcfRepRapFirmware: return {"Default", "MZV", "ZV", "ZVD", "ZVDD", "ZVDDD", "EI2", "EI3", "DAA"};
    case GCodeFlavor::gcfMarlinFirmware: return {"ZV"};
    case GCodeFlavor::gcfMarlinLegacy: return {};
    default: break;
    }
    return {"Default"};
}
...
static t_config_enum_values s_keys_map_GCodeFlavor{{"marlin", gcfMarlinLegacy},
                                                   {"klipper", gcfKlipper},
...
static t_config_enum_values s_keys_map_MicroMoldingType{{"none", int(MicroMoldingType::None)},
...
static t_config_enum_values s_keys_map_NoiseType{{"classic", int(NoiseType::Classic)}, {"perlin", int(NoiseType::Perlin)},
                                                 {"billow", int(NoiseType::Billow)},   {"ridgedmulti", int(NoiseType::RidgedMulti)},
                                                 {"voronoi", int(NoiseType::Voronoi)}, {"ripple", int(NoiseType::Ripple)}};
...
static t_config_enum_values s_keys_map_InputShaperType{{"Default", int(InputShaperType::Default)},
...
static const t_config_enum_values s_keys_map_NozzleVolumeType = {{"Standard", nvtStandard},
...
static const t_config_enum_values s_keys_map_PrimeVolumeMode = {{"Default", pvmDefault}, {"Saving", pvmSaving}};
...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L("Ordered list of bed exclude areas by parallel printhead count. Item 1 applies to one printhead, item 2 to two "
                     "printheads, and so on. Leave an item empty for no excluded area.");
    def->mode    = comAdvanced;
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip  = L(
        "Controls the density (spacing) of external bridge lines. Default is 100%.\n"
         ...
         "  - Pros: Can create a string-like first layer. Faster and with better cooling because there is more space for air to circulate "
         "around the extruded bridge.\n"
    ...
    def->tooltip  = L(
        "Controls the density (spacing) of internal bridge lines. Default is 100%. 100% means a solid internal bridge.\n\n"
         "Internal bridges act as intermediate support between sparse infill and top solid infill and can strongly affect top surface "
         "quality.\n\n"
         "- Higher than 100% density (Recommended Max 125%):\n"
         ...
         "This option works particularly well when combined with the second internal bridge over infill option to improve bridging further "
    ...
    def->tooltip  = L(
        "This value governs the thickness of the external (visible) bridge layer.\n"
         ...
         "The actual bridge flow used is calculated by multiplying this value with the filament flow ratio, and if set, the object's flow "
         "ratio.");
    def->min  = 0;
    def->max  = 2.0;
    def->mode = comAdvanced;
    ...
    def->label       = L("Bridge");
    def->category    = L("Quality");
    def->tooltip     = L("Bridge line width is expressed either as an absolute value or as a percentage of the active nozzle diameter "
                             "(percentages are computed from the nozzle diameter).\n"
    ...
    def->category = L("Quality");
    def->tooltip  = L(
        "This value governs the thickness of the internal bridge layer. This is the first layer over sparse infill so increasing it may "
         "increase strength and upper layer quality.\n"
         "Values above 1.0: Increase the amount of material while maintaining line spacing. This can improve line contact and strength.\n"
         "Values below 1.0: Reduce the amount of material while adjusting line spacing to maintain contact. This can improve sagging.\n\n"
         "The actual bridge flow used is calculated by multiplying this value with the filament flow ratio, and if set, the object's flow "
         "ratio.");
    ...
    def->tooltip = L("Enable this option to slow down printing in areas where perimeters may have curled upwards.\n"
                     ...
                     "due to the potentially large variance in print speeds causing the extruder to be unable to keep up with the "
                     ...
                     "2. Increase the minimum print speeds to limit the magnitude of the slowdown and reduce the variance between fast and "
                     "slow segments.\n"
                     ...
                     "For example, when the perimeters are 100% overhanging, with no wall supporting them from underneath, the 100% "
    ...
    def->tooltip  = L("This factor affects the amount of material for brims.\n\n"
                       "The actual brim flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.\n\n"
    ...
    def->category = L("Quality");
    def->tooltip  = L(
        "The direction which the contour wall loops are extruded when looking down from the top.\nHoles are printed in the opposite "
         "direction to the contour to maintain alignment with layers whose contour polygons are incomplete and change direction, also "
         "partially forming the contour of a hole.\n\nThis option will be disabled if spiral vase mode is enabled.");
    ...
    */
    ...
    def->tooltip  = L("To prevent oozing, the nozzle temperature will be cooled during ramming. Note: only a cooldown command and fan "
                       "activation are triggered, reaching the target temperature is not guaranteed. 0 means disabled.");
    def->mode     = comAdvanced;
    def->sidetext = "°C";
    def->min      = 0;
    def->nullable = true;
    ...
    def->tooltip = L("Shifts the ripple phase forward along the print path by the specified percentage of a wavelength each layer period.\n"
                     ...
                     "The shift is applied once every number of layers set by Layers between ripple offset, so layers within the same "
                     "group are printed identically.");
    def->min     = 0;
    def->max     = 100;
    def->sidetext = ("%");
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("Specifies how many consecutive layers share the same ripple phase before the offset is applied.\n"
                       "For example:\n"
                       "- 1 = Layer 1 is printed with the base ripple pattern, then layer 2 is shifted by the configured offset, then layer "
                       "3 returns to the base pattern, and so on.\n"
                       "- 3 = Layers 1 to 3 are printed with the base ripple pattern, then layers 4 to 6 are shifted by the configured "
                       "offset, then layers 7 to 9 return to the base pattern, etc.");
    def->min      = 1;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionInt(1));
    ...
    def->enum_labels   = {L("Default"), L("MZV"),      L("ZV"),  L("ZVD"),      L("ZVDD"), L("ZVDDD"),  L("EI"),
                          L("EI2"),     L("2HUMP_EI"), L("EI3"), L("3HUMP_EI"), L("DAA"),  L("Disable")};
    def->mode          = comExpert;
    ...
    def->tooltip = L(
        ...
        "\"Full fan speed at layer\" will be ignored if lower than \"For the first\", in which case the fan will run at maximum allowed "
        "speed at layer \"For the first\" + 1.");
    def->min  = 0;
    def->max  = 1000;
    def->mode = comSimple;
    def->set_default_value(new ConfigOptionInts{0});
    ...
    def->category = L("Strength");
    def->tooltip  = L(
        "Embedding the wall into parts where the wall loops are absent ensures that the wall connects seamlessly to the infill.");
    def->set_default_value(new ConfigOptionBool(false));
...
//    def->min = 0;
    ...
    def->category = L("Quality");
    def->tooltip  = L(
        "The maximum deviation allowed when reducing the resolution for the 'Maximum wall resolution' setting. If you increase this, "
...
{
    m_filament_option_keys = {"filament_diameter",
                              "min_layer_height",
                              "max_layer_height",
...
std::set<std::string> print_options_with_variant = {
    ...
    "small_perimeter_threshold", "sparse_infill_speed", "internal_solid_infill_speed", "top_surface_speed",
    "enable_overhang_speed",          // coBools
    "slowdown_for_curled_perimeters", // coBool — per-variant
    "overhang_1_4_speed", "overhang_2_4_speed", "overhang_3_4_speed", "overhang_4_4_speed", "bridge_speed",
    ...
    // --- Acceleration ---
    "default_acceleration", "initial_layer_acceleration",
...
std::set<std::string> filament_options_with_variant =
    {"filament_flow_ratio", "filament_max_volumetric_speed", "filament_ramming_volumetric_speed_nc", "filament_pre_cooling_temperature_nc",
     "filament_ramming_travel_time_nc", "filament_preheat_temperature_delta",
     //"filament_extruder_id",
     "filament_extruder_variant", "filament_retraction_length", "filament_retract_length_nc", "filament_z_hop", "filament_z_hop_types",
     "filament_retract_lift_above", "filament_retract_lift_below", "filament_retract_lift_enforce", "filament_retract_restart_extra",
     ...
     "filament_cooling_before_tower", "volumetric_speed_coefficients", "filament_adaptive_volumetric_speed", "filament_ironing_flow",
     "filament_ironing_spacing", "filament_ironing_inset", "filament_ironing_speed", "activate_air_filtration",
     "activate_air_filtration_during_print", "activate_air_filtration_on_completion", "during_print_exhaust_fan_speed",
     "complete_print_exhaust_fan_speed", "filament_pre_cooling_temperature_nc", "filament_cooling_before_tower",
     "filament_retract_length_nc", "filament_prime_volume", "filament_prime_volume_nc"};
...
void DynamicPrintConfig::normalize_fdm()
{
    ...
    if (this->has("sparse_infill_filament_id")) {
        ...
        if (sparse_infill_filament_id > 0 &&
            (!this->has("internal_solid_filament_id") || this->option("internal_solid_filament_id")->getInt() == 0))
...
void DynamicPrintConfig::save_variant_overrides(int variant_index, const std::set<std::string>& keys)
{
    ...
    if (variant_count == 0) {
        ...
        for (const char* vkey : {"print_extruder_variant", "filament_extruder_variant", "printer_extruder_variant"}) {
            ...
            if (opt && (int) opt->size() > 1) {
                variant_count = (int) opt->size();
                BOOST_LOG_TRIVIAL(info) << "H2C save_variant_overrides: variant_count=" << variant_count << " from " << vkey;
                break;
            }
        }
    }
    ...
    for (const auto& key : keys) {
        ...
        if (!m_variant_overrides.has(key) && variant_count > 0) {
            ...
            case coFloat: {
                double val = static_cast<const ConfigOptionFloat*>(opt)->value;
                m_variant_overrides.floats[key].assign(variant_count, val);
                BOOST_LOG_TRIVIAL(info) << "H2C save_variant_overrides: auto-init " << key << " (coFloat) = " << val << " x"
                                        << variant_count;
                break;
            }
            case coFloatOrPercent: {
                ...
                BOOST_LOG_TRIVIAL(info) << "H2C save_variant_overrides: auto-init " << key << " (coFloatOrPercent) = " << raw << " x"
                                        << variant_count;
                break;
            }
            case coBool: {
                bool val = static_cast<const ConfigOptionBool*>(opt)->value;
                m_variant_overrides.floats[key].assign(variant_count, val ? 1.0 : 0.0);
                BOOST_LOG_TRIVIAL(info) << "H2C save_variant_overrides: auto-init " << key << " (coBool) = " << val << " x"
                                        << variant_count;
                break;
            }
            case coInt: {
                int val = static_cast<const ConfigOptionInt*>(opt)->value;
                m_variant_overrides.floats[key].assign(variant_count, (double) val);
                BOOST_LOG_TRIVIAL(info) << "H2C save_variant_overrides: auto-init " << key << " (coInt) = " << val << " x" << variant_count;
                break;
            }
            default: continue; // unsupported type, skip
            }
...
int DynamicPrintConfig::get_extruder_nozzle_volume_count(int extruder_count,
                                                         std::vector<std::vector<NozzleVolumeType>>& nozzle_volume_types) const
{
...
std::vector<int> DynamicPrintConfig::update_values_to_printer_extruders(DynamicPrintConfig& printer_config,
                                                                        int extruder_count,
                                                                        ...
                                                                        NozzleVolumeType filament_nvt)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(", Line %1%: extruder_count %2%, extruder_nozzle_volume_count %3%") % __LINE__ %
                                   extruder_count % extruder_nozzle_volume_count;
    ...
    // if (extruder_nozzle_volume_count > 1)
    {
        ...
        if (extruder_id > 0 && extruder_id <= static_cast<unsigned>(extruder_count)) {
            ...
            if (nozzle_volume_type == nvtHybrid) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(", Line %1%: nozzle_volume_type is %2%,  not equal to filament_nvt %3%") %
                                               __LINE__ % nozzle_volume_type % filament_nvt;
                // assert(false);
            }
        ...
        for (auto& key : key_set) {
            ...
            if (!this->option(key)) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(", Line %1%: option not present in config for %2%") % __LINE__ % key;
                continue;
            }
            switch (optdef->type) {
            case coStrings: {
                                                            ...
                                                            unsigned int extruder_id)
{
    ...
    if ((extruder_count > 1) || different_extruder) {
        ...
        if (!opt_extruder_type || !opt_nozzle_volume_type) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(", Line %1%: extruder_type or nozzle_volume_type option not found, skipping") %
                                              __LINE__;
            return;
        }
        std::vector<int> variant_index;
        ...
        if (extruder_id > 0 && extruder_id <= static_cast<unsigned>(extruder_count)) {
            ...
            //     }
            //     else {
            //         BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: nozzle_volume_type is default in unsupported
            //         machine.")%__LINE__; assert(false);
            //     }
            // }
            // else if (nozzle_volume_type != filament_nvt) {
            //     if (extruder_nozzle_volume_count > extruder_count) {
            //         BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(", Line %1%: nozzle_volume_type is %2%,  not equal to
            //         filament_nvt %3%")%__LINE__ %nozzle_volume_type %filament_nvt;
            //         //assert(false);
            //     }
            // }
                                                                                   ...
                                                                                   std::string variant_name)
{
    ...
    this->update_values_to_printer_extruders_for_multiple_filaments(printer_config, extruder_count, extruder_nozzle_volume_count, key_set,
                                                                                   ...
                                                                                   std::string variant_name)
{
    ...
    {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", Line %1%: different nozzle volume processing, extruder_count=%2%") % __LINE__ %
                                       extruder_count;
        ...
        for (auto& key : key_set) {
            ...
            case coStrings: {
                ...
                if (!opt) {
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                               << boost::format(", Line %1%: option %2% not found, skipping") % __LINE__ % key;
                    break;
                }
                std::vector<std::string> new_values;
...
std::map<std::string, std::string> validate(const FullPrintConfig& cfg, bool under_cli)
{
    ...
    // extrusion widths
    {
        ...
        const char* widths[] = {"outer_wall_line_width",    "inner_wall_line_width",
                                ...
                                "bridge_line_width",        "top_surface_line_width",
                                "support_line_width",       "initial_layer_line_width",
...
static std::map<t_custom_gcode_key, t_config_option_keys> s_CustomGcodeSpecificPlaceholders{
    ...
    {"change_filament_gcode",
     {"layer_num", "layer_z", "max_layer_z", "next_extruder", "previous_extruder", "fan_speed", "first_flush_volume", "flush_length_1",
      "flush_length_2", "flush_length_3", "flush_length_4", "new_filament_e_feedrate", "new_filament_temp", "new_retract_length",
      "new_retract_length_toolchange", "old_filament_e_feedrate", "old_filament_temp", "old_retract_length",
      "old_retract_length_toolchange", "relative_e_axis", "second_flush_volume", "toolchange_count", "toolchange_z", "travel_point_1_x",
      "travel_point_1_y", "travel_point_2_x", "travel_point_2_y", "travel_point_3_x", "travel_point_3_y", "x_after_toolchange",
      "y_after_toolchange", "z_after_toolchange",
...
CustomGcodeSpecificConfigDef::CustomGcodeSpecificConfigDef()
{
    ...
    new_def("next_hotend", coInt, "Next hotend",
            "Hotend ID being switched to during a tool change. -1 when no nozzle group result is available (single-extruder printers).");
    new_def("old_extruder_variant", coString, "Old extruder variant",
            "Variant string of the extruder being unloaded (e.g. \"Direct Drive Standard\", \"Direct Drive TPU High Flow\"). Empty on cold "
            "start.");
