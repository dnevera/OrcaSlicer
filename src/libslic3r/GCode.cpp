...
static bool is_bambu_x2d_printer(const FullPrintConfig& config) { return config.printer_model.value == "Bambu Lab X2D"; }
    ...
    if (!change_filament_gcode.empty()) {
        ...
        // BBS
        {
            ...
            if (filament_cooling_before_tower.size() < m_print_config->filament_type.values.size())
                filament_cooling_before_tower.resize(m_print_config->filament_type.values.size(),
                                                     m_print_config->filament_cooling_before_tower.get_at(0));
...
std::string WipeTowerIntegration::append_tcr2(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_extruder_id, double z) const
{
    ...
    const bool should_travel_to_tower    = !tcr.priming && (tcr.force_travel     // wipe tower says so
                                                         ...
                                                         || is_ramming || tool_change_on_wipe_tower);
...
static std::vector<std::pair<std::string, std::string>> validate_custom_gcode(const Print& print)
{
    ...
    if (ret.size() < MAX_TAGS_COUNT)
        check(_(L("Process change extrusion role G-code")), config.process_change_extrusion_role_gcode.value);
    if (ret.size() < MAX_TAGS_COUNT)
...
namespace DoExport {
static void init_gcode_processor(const PrintConfig& config,
...
void GCode::_do_export(Print& print, GCodeOutputStream& file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    const bool is_h2c_multi_nozzle = is_bbl_printers && (print.config().nozzle_diameter.size() > 1) &&
    ...
    if (!has_BTT_thumbnail) {
        ...
        bool is_h2c_multi_nozzle = is_bbl_printers && (print.config().nozzle_diameter.size() > 1) &&
                                   (print.config().extruder_max_nozzle_count.values.size() > 1) &&
                                   (print.config().extruder_max_nozzle_count.values[1] > 1);
    ...
    if (is_h2c_multi_nozzle) {
        ...
        this->placeholder_parser().set("current_hotend", -1);
        {
            ...
            if (gr) {
                ...
                this->placeholder_parser().set("initial_nozzle_id", first_nozzle ? (int) first_nozzle->group_id : -1);
    ...
    this->placeholder_parser().set("close_additional_fan_first_x_layers",
                                   new ConfigOptionInts(m_config.close_additional_fan_first_x_layers));
    ...
    this->placeholder_parser().set("filament_pre_cooling_temperature_nc",
                                   new ConfigOptionIntsNullable(print.config().filament_pre_cooling_temperature_nc));
    this->placeholder_parser().set("filament_cooling_before_tower",
                                   new ConfigOptionFloatsNullable(print.config().filament_cooling_before_tower));
 ...
 */
    ...
    if (m_config.support_air_filtration.value) {
        ...
        for (const auto& extruder : m_writer.extruders()) {
            if (m_config.activate_air_filtration.get_at(extruder.id()) &&
                m_config.activate_air_filtration_during_print.get_at(extruder.id())) {
            ...
            if (has_wipe_tower && !layers_to_print.empty()) {
                ...
                if (is_bbl_printers) {
                    if (m_config.default_acceleration.value > 0 && m_config.travel_acceleration.value > 0) {
                        ...
                        file.write(m_writer.set_travel_acceleration((unsigned int) floor(m_config.travel_acceleration.value + 0.5)));
                    }
                    ...
                    file.write(m_writer.travel_to_z(initial_layer_print_height + m_config.z_offset.value + z_hop_for_init,
                                                    "Move to the first layer height"));
                } else {
                    file.write(
                        m_writer.travel_to_z(initial_layer_print_height + m_config.z_offset.value, "Move to the first layer height"));
                }
    ...
    if (m_config.support_air_filtration.value) {
        ...
        for (const auto& extruder : m_writer.extruders()) {
            if (m_config.activate_air_filtration.get_at(extruder.id()) &&
                m_config.activate_air_filtration_on_completion.get_at(extruder.id())) {
...
void GCode::export_layer_filaments(GCodeProcessorResult* result)
{
    ...
    // BBL ref: BambuStudio GCode.cpp:3231-3294 (commit 3f2570c)
    {
        ...
        if (m_sorted_layer_filaments.empty() && m_print->calib_params().mode == CalibMode::Calib_PA_Line) {
            ...
            for (size_t layer_idx = 0; layer_idx < m_sorted_layer_filaments.size(); ++layer_idx) {
                for (auto fidx : m_sorted_layer_filaments[layer_idx]) {
                    int nozzle_idx = group_result ? group_result->get_nozzle_id(fidx, layer_idx) :
                                                    (fidx < filament_map.size() && filament_map[fidx] > 0 ? filament_map[fidx] - 1 : 0);
                    if (!prev_nozzle || !prev_filament || *prev_nozzle != static_cast<unsigned int>(nozzle_idx) || *prev_filament != fidx) {
        ...
        if (group_result) {
            ...
            bool can_compute = !logical_filaments.empty() && !nozzle_list.empty() && !filament_seq.empty() && !nozzle_seq.empty() &&
                               group_count > 0;
            size_t all_filaments_count = m_config.filament_map.values.size();
            std::vector<int> final_assignment(all_filaments_count, 0);
            if (can_compute) {
                auto used_assignment = MultiNozzleUtils::find_optimal_physical_assignment(logical_filaments, nozzle_list, filament_seq,
                                                                                          nozzle_seq, group_count, time_params);
...
size_t GCode::cur_config_index() const { return m_config.filament_map_2.get_at(m_writer.filament()->id()); }
...
void GCode::print_machine_envelope(GCodeOutputStream& file, Print& print)
{
    ...
    if ((flavor == gcfMarlinLegacy || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware) &&
        ...
        if (print.config().input_shaping_emit.value && flavor != gcfMarlinLegacy) {
            ...
            file.write_format(writer()
                                  .set_input_shaping('X', print.config().input_shaping_damp_x.value,
                                                     print.config().input_shaping_freq_x.value,
                                                     print.config().opt_serialize("input_shaping_type"))
                                  .c_str());
            if (flavor != gcfRepRapFirmware && !input_shaping_disable) {
                file.write_format(
                    writer()
                        .set_input_shaping('Y', print.config().input_shaping_damp_y.value, print.config().input_shaping_freq_y.value, "")
                        .c_str());
            }
        }
    }
                                 ...
                                 const bool prime_extruder)
{
    ...
    case CalibMode::Calib_Temp_Tower: {
        gcode += writer().set_temperature(this->interpolate_value_across_layers(static_cast<float>(print.calib_params().start),
                                                                                static_cast<float>(print.calib_params().end), 5.0f));
        break;
    }
                ...
                for (const ObjectByExtruder::Island::Region::Type entity_type :
                    ...
                    for (const ExtrusionEntity* ee :
                        ...
                        auto process_extrusions = [&](const ExtrusionEntityCollection* current_extrusions,
                                                      const ExtrusionEntityCollection* overrides_key, bool use_overrides) {
                            ...
                            if (is_anything_overridden && use_overrides) {
                                entity_overrides = const_cast<LayerTools&>(layer_tools)
                                                       .wiping_extrusions()
                                                       .get_extruder_overrides(overrides_key, layer_to_print.original_object,
                                                                               correct_extruder_id,
                                ...
                                if (entity_overrides == nullptr) {
                                    ...
                                    for (int extruder : *entity_overrides)
                                        printing_extruders.emplace_back(
                                            extruder >= 0 ?
                                ...
                                for (size_t i = 0; i <= n_slices; ++i) {
                                    ...
                                    if (last || point_inside_surface(island_idx, current_extrusions->first_point())) {
                                        if (islands[island_idx].by_region.empty())
                                            islands[island_idx].by_region.assign(print.num_print_regions(),
                        ...
                        bool split_mixed_perimeters = entity_type == ObjectByExtruder::Island::Region::PERIMETERS &&
    ...
    if (object_layer && !object_layer->micro_molding_injection_points.empty()) {
        ...
        gcode += MicroMolding::generate_injection_gcode(layer.object(), object_layer->id(), print_z, object_layer->height, filament_dia,
                                                        nozzle_dia, nozzle_temp, m_config.use_relative_e_distances.value, copy_offset);

        // Post-injection nozzle cleaning (logic in MicroMolding.cpp)
        {
            int plate_idx = print.get_plate_index();
            gcode += MicroMolding::generate_post_injection_cleanup(print_z, print.config().wipe_tower_x.get_at(plate_idx),
...
void GCode::append_full_config(const Print& print, std::string& str)
{
    ...
    {
        auto fmt_vec_opt = [](const ConfigOption* opt) {
            if (!opt)
                return std::string("(null)");
            auto* v = dynamic_cast<const ConfigOptionInts*>(opt);
            if (!v)
                return std::string("(not_ints)");
            std::string s = "[";
...
void GCode::set_origin(const Vec2d& pointf)
{
    ...
    const Point3 translate(scale_(m_origin(0) - pointf(0)), scale_(m_origin(1) - pointf(1)));
...
void GCode::precompute_extruder_speed_overrides(const Print& print)
{
    ...
    for (unsigned int eid = 0; eid < num_extruders; ++eid) {
        ...
        if (has_any) {
            ...
            BOOST_LOG_TRIVIAL(info) << "H2C precompute: built overlay for extruder " << eid << " (variant_index=" << variant_index
                                ...
                                const Point* start_point)
{
    ...
    if (m_wipe.enable && FILAMENT_CONFIG(wipe)) {
        ...
        for (ExtrusionPath& path : paths) {
            ...
            if (!m_wipe.path.empty() && !path.empty() && m_wipe.path.last_point() == Point(path.first_point().x(), path.first_point().y())) {
...
std::string GCode::_extrude(const ExtrusionPath& path, std::string description, double speed)
{
    ...
    if (!m_last_pos_defined || m_last_pos.to_point() != first_point || m_need_change_layer_lift_z || slope_need_z_travel) {
        ...
        if (!slope_need_z_travel && (_last_pos_undefined || m_need_change_layer_lift_z)) {
            const std::string z_sync_comment = _last_pos_undefined ? "ensure Z matches planned layer height" :
    ...
    if (this->on_first_layer() || object_layer_over_raft()) {
        ...
        if (path.role() != erBottomSurface) {
            speed = (is_perimeter(path.role()) || path.role() == erGapFill) ? m_config.get_abs_value("initial_layer_speed") :
        ...
        if (_layer > 0 && _layer < m_config.slow_down_layers) {
            const auto first_layer_speed = (is_perimeter(path.role()) || path.role() == erGapFill) ?
    ...
    if (path.role() != m_last_processor_extrusion_role) {
        ...
        if (is_BBL_Printer() && m_config.nozzle_diameter.size() > 1 && m_layer_index >= 2 &&
            m_last_processor_extrusion_role == erPerimeter && path.role() == erExternalPerimeter) {
                ...
                for (size_t fitting_index = 0; fitting_index < fitting_result.size(); fitting_index++) {
                    ...
                    case EMovePathType::Linear_move: {
                        ...
                        for (size_t point_index = start_index + 1; point_index < end_index + 1; point_index++) {
                            ...
                            const Line line          = Line(path.polyline.points[point_index - 1].to_point(),
                                                            path.polyline.points[point_index].to_point());
                            const double line_length = line.length() * SCALING_FACTOR;
                            if (line_length < EPSILON)
                                continue;
                            auto dE = e_per_mm * line_length;
...
std::string GCode::set_extruder(unsigned int new_filament_id, double print_z, bool by_object, int toolchange_temp_override)
{
    ...
    if (m_writer.filament() != nullptr || m_start_gcode_filament != -1) {
        ...
        if (layered_gr) {
            ...
            if (old_extruder_id != new_extruder_id ||
                !layered_gr->are_filaments_same_nozzle(old_filament_id, new_filament_id, m_layer_index)) {
    ...
    if (filament_cooling_before_tower.size() < m_print->config().filament_type.values.size())
        filament_cooling_before_tower.resize(m_print->config().filament_type.values.size(),
                                             m_print->config().filament_cooling_before_tower.get_at(0));
    std::fill(filament_cooling_before_tower.begin(), filament_cooling_before_tower.end(), 0);
...
void GCode::update_placeholder_parser_with_variant_params()
{
    ...
    this->placeholder_parser().set("filament_max_volumetric_speed",
                                   new ConfigOptionFloats(remap_floats_by_filament(m_config.filament_max_volumetric_speed)));
    this->placeholder_parser().set("filament_pre_cooling_temperature_nc",
                                   new ConfigOptionInts(remap_ints_by_filament(m_config.filament_pre_cooling_temperature_nc)));
    this->placeholder_parser().set("filament_cooling_before_tower",
                                   new ConfigOptionFloats(remap_floats_by_filament(m_config.filament_cooling_before_tower)));
    this->placeholder_parser().set("nozzle_temperature_initial_layer",
                                   new ConfigOptionInts(remap_ints_by_filament(m_config.nozzle_temperature_initial_layer)));
    ...
    this->placeholder_parser().set("retraction_distances_when_cut",
                                   new ConfigOptionFloats(remap_floats_by_filament(m_config.retraction_distances_when_cut)));
