...
#define RETURN_COMPARE_NON_EQUAL_TYPED(TYPE, KEY) \
    ...
    bool operator==(const SurfaceFillParams& rhs) const
    {
        return this->extruder == rhs.extruder && this->pattern == rhs.pattern && this->spacing == rhs.spacing &&
               ...
               this->extrusion_role == rhs.extrusion_role && this->role_speed == rhs.role_speed &&
               this->first_layer_flow_ratio == rhs.first_layer_flow_ratio && this->lateral_lattice_angle_1 == rhs.lateral_lattice_angle_1 &&
...
void split_solid_surface(size_t layer_id, const SurfaceFill& fill, ExPolygons& normal_infill, ExPolygons& narrow_infill)
{
    ...
    const bool line_based_pattern = fill.params.pattern == ipRectilinear || fill.params.pattern == ipMonotonic ||
        ...
        for (ExPolygon& expoly : surface_fill.expolygons) {
                ...
                if (z_amp_pct > 0) {
                    ...
                    for (const Layer *adj : all_layers) {
                        if (adj == this || adj->print_z < z_lo || adj->print_z > z_hi)
                            continue;
                        // Union all regions' fill areas first, then
                        // intersect once.  Sequential per-region
                        // intersection is wrong for multi-material:
                        // e.g. PLA-Matte walls + PLA-CF infill cover
                        // different non-overlapping areas.
                        ExPolygons adj_fill;
                        for (const LayerRegion *adj_region : adj->regions()) {
                            if (!adj_region->fill_no_overlap_expolygons.empty()) {
                                adj_fill.insert(adj_fill.end(),
                                    adj_region->fill_no_overlap_expolygons.begin(),
                                    adj_region->fill_no_overlap_expolygons.end());
                            }
                        }
                        if (!adj_fill.empty()) {
            ...
            if (!is_approx(elefant_density, 1.0f) && surface_fill.surface.is_solid_infill()) {
                ...
                if (f->layer_id > 0 && f->layer_id <= elefant_layers)
                    params.density = 1.0f - (1.0f - elefant_density) * (elefant_layers - (f->layer_id - 1)) /
                                                elefant_layers; // Reverse calculation - The higher layer number means the higher density.
                                                                // Counting starts from the second layer.
            }
            // make fill
...
void Layer::make_ironing()
{
    ...
    for (LayerRegion* layerm : m_regions)
        if (!layerm->slices.empty()) {
            ...
            if (ironing_params.extruder != -1) {
                ...
                double ironing_angle = (config.ironing_angle_fixed ?
                                            0 :
    ...
    for (size_t i = 0; i < by_extruder.size();) {
        ...
        f->dont_alternate_fill_direction = ironing_params.layerm->region().config().zaa_enabled &&
