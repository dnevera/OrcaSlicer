...
std::vector<int> PartPlate::get_extruders(bool conside_custom_gcode) const
{
    ...
    if (glb_outer_wall_extr == 0)
        glb_outer_wall_extr = glb_inner_wall_extr;
    if (glb_inner_wall_extr == 0)
        glb_inner_wall_extr = glb_outer_wall_extr;
    ...
    if (glb_top_surface_extr == 0)
        glb_top_surface_extr = glb_internal_solid_extr;
    if (glb_bottom_surface_extr == 0)
        glb_bottom_surface_extr = glb_internal_solid_extr;
...
bool PartPlate::check_filament_printable(const DynamicPrintConfig& config, wxString& error_message)
{
    ...
    if (!used_filaments.empty()) {
        ...
        const auto &filament_printables = filament_printable_opt->values;
        // By-value: get_real_filament_maps() may return a temporary on H2C paths.
        ...
        // Pre-compute the safe upper bound across all per-filament arrays (SoftFever #14103).
        const int filament_count = std::min({(int) filament_types.size(),
                                             (int) filament_printables.size(),
                                             (int) filament_map.size()});
                          ...
                          float height_to_rod)
{
    ...
    if ((m_shape != new_shape) || (m_exclude_area != new_exclude_areas)) {
        ...
        if (m_plater != nullptr) { // render data, skip in CLI mode where m_plater is null
            ...
            // ORCA also change bed_icon_count number in calc_vertex_for_icons() after adding or removing icons for circular shaped beds
            // that uses vertical alingment for icons
            bool dual_bbl        = false;
            PresetBundle* preset = wxGetApp().preset_bundle;
...
void PartPlateList::set_default_wipe_tower_pos_for_plate(int plate_idx, bool init_pos)
{
    ...
    if (wrapping_opt)
        enable_wrapping = wrapping_opt->value;
    int nozzle_nums       = wxGetApp().preset_bundle->get_printer_extruder_count();
