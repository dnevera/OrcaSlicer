...
static NozzleVolumeType flow_to_volume(NozzleFlowType flow)
{
    return (flow == NozzleFlowType::H_FLOW) ? nvtHighFlow : nvtStandard;
}

// Aggregate the active (in-extruder) and rack nozzles into per-(extruder, diameter,
// flow) groups. Extruder-id convention matches VortekDeviceHooks: active nozzle id 0
// is the left extruder, everything else (and all rack nozzles) is the right extruder.
static std::vector<MultiNozzleUtils::NozzleGroupInfo> BuildNozzleGroups(const std::shared_ptr<VortekNozzleRack>& rack)
{
    std::vector<MultiNozzleUtils::NozzleGroupInfo> groups;
    if (!rack)
        return groups;

    if (auto* system = rack->GetNozzleSystem()) {
        for (const auto& elem : system->GetNozzles()) {
            const auto& nozzle = elem.second;
            MultiNozzleUtils::NozzleGroupInfo info;
            info.extruder_id = (nozzle.m_nozzle_id == 0) ? LeftExtruderIdx : RightExtruderIdx;
            info.diameter    = MultiNozzleUtils::format_diameter_to_str(nozzle.m_diameter);
            info.volume_type = flow_to_volume(nozzle.m_nozzle_flow);
            info.nozzle_count = 1;
            groups.emplace_back(std::move(info));
        }
    }

    for (const auto& elem : rack->GetRackNozzles()) {
        const auto& nozzle = elem.second;
        // skip unknown / abnormal nozzles
        if (nozzle.m_nozzle_id < 0 || nozzle.m_nozzle_type == ntUndefine || nozzle.m_diameter < 0.01f)
            continue;
        int              extruder_id = RightExtruderIdx;
        std::string      diameter    = MultiNozzleUtils::format_diameter_to_str(nozzle.m_diameter);
        NozzleVolumeType volume_type = flow_to_volume(nozzle.m_nozzle_flow);

        bool found = false;
        for (auto& group : groups) {
            if (group.extruder_id == extruder_id && group.volume_type == volume_type && group.diameter == diameter) {
                found = true;
        ...
        if (!found)
            groups.emplace_back(diameter, volume_type, extruder_id, 1);
    }
    return groups;
}
    ...
    : GUI::DPIDialog(parent, wxID_ANY, "Set nozzle count", wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX), m_volume_type(volume_type)
{
    ...
    // v2 has no nvtHybrid: a given extruder is either Standard or High Flow.
    bool show_standard = (volume_type == nvtStandard);
    bool show_highflow = (volume_type == nvtHighFlow);
...
void HotEndTable::UpdateRackInfo(std::weak_ptr<VortekNozzleRack> rack)
{
    m_nozzle_rack = rack;
...
std::vector<int> HotEndTable::FilterHotEnds(const NozzleOption& option)
{
    ...
    // The rack holds right-extruder nozzles only; highlight those matching the
    // option's diameter and requested flow.
    std::set<NozzleVolumeType> wanted;
    for (const auto& item : option.extruder_nozzle_stats) {
        for (const auto& nz : item.second) {
            if (nz.second > 0)
                wanted.insert(nz.first);
        }
    }
    ...
    for (const auto& elem : rack->GetRackNozzles()) {
        ...
        if (!rack->IsNozzleOnRack(nozzle.m_nozzle_id))
            continue;
        if (MultiNozzleUtils::format_diameter_to_str(nozzle.m_diameter) != option.diameter)
            continue;
        if (wanted.count(flow_to_volume(nozzle.m_nozzle_flow)))
            filtered_nozzles.emplace_back(nozzle.m_nozzle_id);
    }
...
void HotEndTable::UpdateNozzleItems(const std::unordered_map<int, wgtDeviceNozzleRackNozzleItem*>& nozzle_items, std::shared_ptr<VortekNozzleRack> nozzle_rack)
{
...
void MultiNozzleStatusTable::UpdateRackInfo(std::weak_ptr<VortekNozzleRack> rack)
{
    if (m_table)
    ...
    if (m_badge) {
        ...
        if (!nozzle_rack)
            return;
        auto* system = nozzle_rack->GetNozzleSystem();
        if (!system)
            return;
        // active (in-extruder) nozzles: id 0 -> left, else right.
        bool has_right = false;
        for (const auto& elem : system->GetNozzles()) {
            const auto& nozzle = elem.second;
            int extruder_id = (nozzle.m_nozzle_id == 0) ? LeftExtruderIdx : RightExtruderIdx;
            if (extruder_id == RightExtruderIdx)
                has_right = true;

            NozzleVolumeType volume_type = flow_to_volume(nozzle.m_nozzle_flow);

            m_badge->SetExtruderInfo(extruder_id, MultiNozzleUtils::format_diameter_to_str(nozzle.m_diameter), volume_type);
        }
...
Slic3r::GUI::MultiNozzleSyncDialog::MultiNozzleSyncDialog(wxWindow* parent,std::weak_ptr<VortekNozzleRack> rack) : DPIDialog(parent, wxID_ANY, _L("Sync Nozzle status"),wxDefaultPosition, wxDefaultSize,wxDEFAULT_DIALOG_STYLE)
{
    m_nozzle_rack = rack;
    wxSizer* main_sizer = new wxBoxSizer(wxVERTICAL);
...
void MultiNozzleSyncDialog::UpdateRackInfo(std::weak_ptr<VortekNozzleRack> rack)
{
    m_nozzle_rack = rack;
    UpdateUi(rack);
}
...
bool MultiNozzleSyncDialog::UpdateOptionList(std::weak_ptr<VortekNozzleRack> rack, bool ignore_unknown, bool ignore_unreliable)
{
    const auto& nozzle_rack = rack.lock();
    if (!nozzle_rack)
        return true;
    ...
    auto options = GetNozzleOptions(BuildNozzleGroups(nozzle_rack));
...
void MultiNozzleSyncDialog::UpdateTip(std::weak_ptr<VortekNozzleRack> rack, bool ignore_unknown, bool ignore_unreliable)
{
    const auto& nozzle_rack = rack.lock();
    if (!nozzle_rack)
        return;
...
void MultiNozzleSyncDialog::UpdateButton(std::weak_ptr<VortekNozzleRack> rack, bool ignore_unknown, bool ignore_unreliable)
{
    const auto& nozzle_rack = rack.lock();
    if (!nozzle_rack)
        return;
...
bool MultiNozzleSyncDialog::UpdateUi(std::weak_ptr<VortekNozzleRack> rack, bool ignore_unknown, bool ignore_unreliable)
{
