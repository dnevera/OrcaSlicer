...
void TabPrint::build()
{
    ...
    optgroup->append_single_option_line("micro_molding_flow_multiplier");
    optgroup->append_single_option_line("micro_molding_temp_offset");
    optgroup->append_single_option_line("micro_molding_lock_ratio");
    optgroup->append_single_option_line("micro_molding_neck_layers");
    optgroup->append_single_option_line("micro_molding_head_layers");
    ...
    for (auto page : m_pages) {
        ...
        for (auto group : page->m_optgroups) {
            for (auto& opt : group->opt_map()) {
                ...
                else if (m_extruder_switch && m_type == Preset::TYPE_PRINT && print_options_with_variant.count(opt.second.first)) {
                    const_cast<int&>(opt.second.second) = index;
                    page->m_opt_id_map.insert({opt.second.first + "#" + std::to_string(index), opt.first});
                }
            }
