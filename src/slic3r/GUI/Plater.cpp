    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    p->m_panel_filament_title->Bind(wxEVT_LEFT_UP, [this](wxMouseEvent &e) {
        ...
        // Vortek: also exclude purge mode button area
        // Reference to BBS: BambuStudio/src/slic3r/GUI/Plater.cpp Sidebar::Sidebar
        if (p->m_purge_mode_btn && p->m_purge_mode_btn->IsShown())
            exclude_pt = std::min(exclude_pt, p->m_purge_mode_btn->GetPosition().x);
